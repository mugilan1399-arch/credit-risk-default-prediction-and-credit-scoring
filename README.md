...
2. Split off a stratified validation set from `cs-training.csv`.
3. Realised that:
...
5. Investigated *why* there is a gap, rather than accepting it:
   - The three late-payment count columns showed high pairwise correlation (0.98–0.99), traced to 214 rows sharing an identical value (98 or 96) across all three       columns — a data error, not real payment history. Flagging and nulling these rows fixed both a counterintuitive negative coefficient and pushed logistic           regression to **AUC = 0.854** — nearly matching XGBoost's 0.859.
